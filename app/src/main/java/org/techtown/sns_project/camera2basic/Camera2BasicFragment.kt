...
class Camera2BasicFragment : Fragment(), View.OnClickListener,
    ...
    /*private val onImageAvailableListener = ImageReader.OnImageAvailableListener {
          backgroundHandler?.post(imageUpload(it.acquireNextImage()))
    }*/
    public val onImageAvailableListener = ImageReader.OnImageAvailableListener {

    }

    private class imageUpload(private val image: Image) : Runnable {
        override fun run() {
            ...
            val mountainImagesRef = storageRef.child("images/" + user?.uid + "/profileImage.jpg")

            var uploadTask = mountainImagesRef.putBytes(bytes)
    ...
    private fun setUpCameraOutputs(width: Int, height: Int) {
        ...
        try {
            for (cameraId in manager.cameraIdList) {
                ...
                val swappedDimensions = displayRotation?.let { areDimensionsSwapped(it) }

                val displaySize = Point()
                requireActivity().windowManager.defaultDisplay.getSize(displaySize)
                val rotatedPreviewWidth = if (swappedDimensions == true) height else width
                val rotatedPreviewHeight = if (swappedDimensions == true) width else height
                var maxPreviewWidth = if (swappedDimensions == true) displaySize.y else displaySize.x
                var maxPreviewHeight = if (swappedDimensions == true) displaySize.x else displaySize.y
        ...
        try {
            ...
            val captureCallback = object : CameraCaptureSession.CaptureCallback() {
                ...
                override fun onCaptureCompleted(session: CameraCaptureSession,
                        request: CaptureRequest,
                        result: TotalCaptureResult) {
                    activity?.showToast("사진 촬영이 완료되었습니다.")
